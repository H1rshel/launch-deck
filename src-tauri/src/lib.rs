...
async fn set_console_fullscreen(window: tauri::WebviewWindow, enabled: bool) -> Result<(), String> {
    // Mobile has no window-manager fullscreen/maximize concepts — the webview
    // always fills the screen, so this is a no-op there.
    #[cfg(mobile)]
    {
...
pub fn run() {
    ...
    // Desktop-only: mobile OSes are inherently single-instance.
    #[cfg(desktop)]
    let builder = builder
